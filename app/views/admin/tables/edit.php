...
<div class="bg-white rounded-xl shadow-sm max-w-2xl">
    <div class="p-6 border-b border-gray-200">
        <h2 class="text-xl font-semibold text-gray-800">Editar Mesa</h2>
        <p class="text-gray-600"><?= htmlspecialchars($restaurant['name']) ?></p>
    </div>
    
    <form method="POST" action="<?= BASE_URL ?>/admin/tables/<?= $table['id'] ?>/edit" class="p-6 space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Número/Nombre de Mesa *</label>
                <input type="text" name="table_number" required value="<?= htmlspecialchars($table['table_number']) ?>"
            ...
            <div>
                ...
                <select name="area_id"
                    ...
                    <option value="<?= $area['id'] ?>" <?= $table['area_id'] == $area['id'] ? 'selected' : '' ?>>
            ...
            <div>
                ...
                <input type="number" name="capacity" required min="1" max="50" value="<?= htmlspecialchars($table['capacity']) ?>"
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary">
            </div>
            
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Capacidad mínima</label>
                <input type="number" name="min_capacity" min="1" max="50" value="<?= htmlspecialchars($table['min_capacity']) ?>"
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary">
            </div>
            ...
            <div>
                ...
                <select name="shape"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary">
                    <option value="square" <?= $table['shape'] === 'square' ? 'selected' : '' ?>>Cuadrada</option>
                    ...
                    <option value="other" <?= $table['shape'] === 'other' ? 'selected' : '' ?>>Otra</option>
                </select>
            </div>
        </div>
        ...
        <div>
            ...
            <textarea name="notes" rows="2" placeholder="Notas adicionales sobre la mesa..."
                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary"><?= htmlspecialchars($table['notes'] ?? '') ?></textarea>
        </div>
        
        <div class="space-y-3">
            <label class="flex items-center">
                <input type="checkbox" name="is_combinable" value="1" <?= $table['is_combinable'] ? 'checked' : '' ?>
            ...
            <label class="flex items-center">
                <input type="checkbox" name="is_active" value="1" <?= $table['is_active'] ? 'checked' : '' ?>
                       class="rounded border-gray-300 text-primary focus:ring-primary">
                <span class="ml-2 text-gray-700">Mesa activa</span>
            </label>
        </div>
